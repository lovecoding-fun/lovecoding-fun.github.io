...
今天在腾讯[IMWeb前端博客](http://imweb.io/)中看到了两篇介绍 `React hook` 的文章。受益匪浅。
...
优化问题真是我一个痛点。这边文章很清楚地解释了 `Hooks` 一些可用于组件优化的API。强推！
特别地，文章介绍了当函数组件中传入的 `props` 值为函数时，由于每一次执行或重新执行，作用域里面一切都是重新开始，函数不是简单数据类型，不能画上等号，子组件都会重新渲染。文章提出了几种解决办法：
...
```
`Symbol()` 返回的每个 `symbol` 值都是唯一的，可以接受一个字符串作为参数。它最常被用户对象属性的标识符，如：
```
const obj = {}
